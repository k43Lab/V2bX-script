...
install() {
    bash <(curl -Ls https://proxy-gh.1l1.icu/raw.githubusercontents.com/114514-homo-lab/V2bX-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://proxy-gh.1l1.icu/raw.githubusercontent.com/114514-homo-lab/V2bX-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://proxy-gh.1l1.icu/github.com/ylx2016/Linux-NetSpeed/raw/master/tcpx.sh)
}
